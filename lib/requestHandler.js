...
//
// Background: transparent proxy (PassWall/xray) encapsulates TCP into gRPC streams.
// gRPC streams can be RST_STREAM'd independently, but the TCP socket looks alive
// to Node.js. Reusing a socket whose gRPC stream is dead → ECONNRESET (~20% failure).
//
// Strategy: enable keep-alive via custom agents with maxRequestsPerSocket=50.
// Connections are recycled before gRPC streams go stale, yielding ~50x reduction
// in TCP+TLS handshakes compared to agent:false. On ECONNRESET, retry once with
// a fresh connection as safety net.
 ...
 * POST/PUT/PATCH/DELETE may have already been partially processed upstream
 * when the local socket sees ECONNRESET from a stale gRPC stream.
 */
  ...
  function _doRequest(useAgent) {
    return new Promise((resolve, reject) => {
      ...
      const proxyReq = (isHttps ? https : http).request(opts, (res) => {
        ...
        let rawResChunks = []; // the original response chunks (set to null in stream mode)
        ...
        //deal response data
        res.on('data', (chunk) => {
          if (resDataStream) { // stream mode — promise already resolved, stream directly
    ...
    */
    ...
    co(fetchReqData)
      ...
      .then(co.wrap(function *(userConfig) {
        ...
        // 否则以 64k 为上线，只要返回的数据达到阈值，就直接返回给调用端，以提高性能
