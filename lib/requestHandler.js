...
function fetchRemoteResponse(protocol, options, reqData, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 禁用 Node.js Agent 连接池，每次请求创建全新 TCP 连接。
    //
    // 默认情况下 http.request 使用全局 Agent 做连接池复用（keep-alive），
    // Agent 基于 TCP 连接是否存活来判断是否可复用。但在透明代理 + gRPC 隧道
    // 场景下存在语义断层：
    //
    //   浏览器 → AnyProxy MITM → [TCP] → PassWall 透明代理 → [Trojan + gRPC] → CDN → 目标
    //
    // AnyProxy 发出的 "TCP 连接" 实际被 iptables 重定向到 PassWall，最终封装
    // 进 gRPC stream。gRPC 基于 HTTP/2 stream multiplexing，CDN 或 gRPC 服务端
    // 可以随时 RST_STREAM 关闭单个 stream（超时、限流等），但底层 TCP 连接
    // 对 AnyProxy 来说仍然是 ESTABLISHED 状态。Agent 尝试复用这个"存活"的 TCP
    // 连接时，对应的 gRPC stream 已死，直接收到 ECONNRESET。
    //
    // gRPC 协议下约 20% 请求复用到已关闭 stream 导致失败。
    // ws（WebSocket）和 xhttp 无此问题：ws 是单路隧道，生命周期一一对应；
    // xhttp 内置了请求级 stream 隔离。
    //
    // 设为 false 后跳过 Agent，每个请求独立建连，对应独立的 gRPC stream，
    // 从根源避开 stream 复用问题。代价是每次请求多一次 TCP + TLS 握手。
    options.agent = false;
